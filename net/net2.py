...
flags = tf.app.flags
flags.DEFINE_float("learning_rate", 1e-5, "Learning rate of for adam [0.0002]")
...
class SimilarityNet(object):
    ...
    def __build_model(self):
        ...
        self.dense1 = tf.layers.dense(inputs=self.spliced_fingerprint, units=8192, activation=tf.nn.sigmoid,
                                      kernel_initializer=tf.truncated_normal_initializer(stddev=FLAGS.stddev))
        self.dense2 = tf.layers.dense(inputs=self.dense1, units=1024, activation=tf.nn.sigmoid,
                                      kernel_initializer=tf.truncated_normal_initializer(stddev=FLAGS.stddev))
        self.dense3 = tf.layers.dense(inputs=self.dense2, units=2, activation=tf.nn.sigmoid,
                                      kernel_initializer=tf.truncated_normal_initializer(stddev=FLAGS.stddev))
    ...
    def __accuracy(self):
        ...
        self.equals = tf.equal(tf.reshape(tf.cast(self.label, tf.int64), [-1, 1]), self.label_pred)
        ...
        for i in range(count):
            ...
            print(
                "Loss:{:8.4f},Accuracy:{:8.4f},class1={:8.4f},class2={:8.4f}".format(loss, accuracy, dense3[0][0],
                                                                                     dense3[0][1]))
            print("y_:", label_pred, "y:", batch_label)
